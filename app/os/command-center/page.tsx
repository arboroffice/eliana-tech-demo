...
export default function ElianaChatOS() {
  ...
  // ── Chat send handler (preserved from original) ──
  const handleSendMessage = async () => {
    ...
    await fetch('/api/os/agent-feed', {
      method: 'POST',
      body: JSON.stringify({ level: 'SCAN', message: `New mission request received: "${currentInput}". Initializing cluster...`, missionId, agentId: 'Agent-PM-04' })
    })
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      await fetch('/api/os/agent-feed', {
        method: 'POST',
        body: JSON.stringify({ level: steps[i].level, message: steps[i].msg, missionId })
      })
    }
    ...
    await fetch('/api/os/agent-feed', {
      method: 'POST',
      body: JSON.stringify({ level: 'META', message: `MISSION ${missionId} RESOLVED. Agents returning to idle.`, missionId })
    })
  }
          ...
          <div key={agent.id} className="bg-zinc-900/60 border border-zinc-800 rounded-2xl p-5 hover:border-zinc-700 transition-colors">
            ...
            <div className="mt-3 flex items-center gap-2">
              <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${
                ...
                agent.status === 'training' ? 'bg-amber-600/20 text-amber-400' :
    ...
    <div className="flex flex-col h-full overflow-hidden">
        ...
        {messages.map((msg) => (
          ...
          >
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
              msg.role === 'user' ? 'bg-zinc-800' : 'bg-red-600 shadow-[0_0_20px_rgba(220,38,38,0.3)]'
            }`}>
              {msg.role === 'user' ? <User className="w-5 h-5" /> : <Bot className="w-5 h-5" />}
            </div>

            <div className={`max-w-[70%] space-y-4 ${msg.role === 'user' ? 'text-right items-end flex flex-col' : ''}`}>
              <div className={`p-6 rounded-3xl leading-relaxed text-sm ${
                msg.role === 'user'
                  ? 'bg-red-600 text-white font-bold'
                  : 'bg-zinc-900/80 border border-zinc-800 text-zinc-300 backdrop-blur-md'
              }`}>
                {msg.content}
              </div>
              ...
              {msg.viewportSnapshot && (
                <motion.div
                  ...
                  className="relative rounded-2xl overflow-hidden border border-zinc-800 aspect-video ring-4 ring-black shadow-2xl group"
                >
                  <img src={msg.viewportSnapshot} alt="Browser Native View" className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-4">
                    <div className="flex items-center gap-2 mb-1">
                      <Search className="w-3 h-3 text-red-500" />
                      <span className="text-[10px] text-zinc-400 uppercase font-bold tracking-widest">Vision Logic Analysis</span>
                    </div>
                    <div className="h-1 w-full bg-zinc-800 rounded-full overflow-hidden">
                      <div className="h-full bg-red-600 animate-pulse" style={{ width: '40%' }} />
                    </div>
                  </div>
                  <div className="absolute top-2 right-2 px-2 py-0.5 bg-black/80 rounded font-mono text-[8px] text-zinc-500 uppercase tracking-widest border border-zinc-800">
                    Browser_Native_01
                  </div>
                </motion.div>
              )}
              ...
              {msg.status && msg.status !== 'completed' && (
                <div className="bg-black/40 border border-zinc-800 rounded-2xl p-6 w-full space-y-4">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-[9px] uppercase font-black text-red-500 flex items-center gap-2">
                      <Sparkles className="w-3 h-3 animate-pulse" /> Agent Thinking
                    </span>
                    <span className="text-[9px] font-mono text-zinc-500">{msg.progress}%</span>
                  </div>
                  <div className="h-0.5 bg-zinc-800 rounded-full overflow-hidden">
                    <motion.div className="h-full bg-red-600" initial={{ width: 0 }} animate={{ width: `${msg.progress}%` }} />
                  </div>
                  <div className="space-y-2">
                    {(msg.logs || []).map((log, i) => (
                      <motion.div key={i} initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }} className="text-[10px] text-zinc-500 font-mono flex gap-2">
                        <span className="text-red-900">▸</span> {log}
                      </motion.div>
                    ))}
                  </div>
                </div>
              ...
              {msg.result && (
                <motion.div
                  ...
                  animate={{ opacity: 1, scale: 1 }}
                  className="bg-zinc-900 border border-red-500/20 shadow-[0_0_40px_rgba(220,38,38,0.05)] p-6 rounded-3xl w-full"
                >
                  <div className="flex items-center gap-4 mb-4">
                    <div className="p-2 bg-red-600/10 rounded-lg">
                      <Rocket className="w-4 h-4 text-red-500" />
                    </div>
                    <h5 className="text-xs font-black uppercase text-white tracking-widest">{msg.result.type}</h5>
                  </div>
                  <p className="text-xs text-zinc-400 mb-6 italic">&quot;{msg.result.summary}&quot;</p>
                  <div className="flex justify-between items-center">
                    <span className="text-[9px] text-zinc-600 font-mono italic">{msg.result.evidence}</span>
                    <a href={msg.result.url} className="text-[9px] text-red-500 font-black uppercase hover:text-white transition-colors border-b border-red-500/30 pb-0.5">
                      View Infrastructure Output
                    </a>
                  </div>
                </motion.div>
              )}
      ...
      <div className="p-8 bg-gradient-to-t from-black to-transparent shrink-0">
        <div className="relative group max-w-4xl mx-auto">
          <div className="absolute -inset-1 bg-gradient-to-r from-red-600/20 to-transparent rounded-2xl blur opacity-25 group-hover:opacity-100 transition duration-1000" />
          <div className="relative bg-zinc-900 border border-zinc-800 rounded-2xl flex items-center p-2 shadow-2xl">
            <input
              type="text"
              value={missionInput}
              onChange={(e) => setMissionInput(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()}
              placeholder="Ask Eliana-OS anything... (e.g., 'Optimize my sales follow-ups')"
              className="bg-transparent border-none flex-grow p-4 focus:ring-0 focus:outline-none text-sm text-zinc-300 placeholder:text-zinc-600 font-medium"
            />
            <button onClick={handleSendMessage} className="bg-red-600 hover:bg-white hover:text-black transition-all p-4 rounded-xl shadow-[0_0_15px_rgba(220,38,38,0.4)]">
              <Send className="w-4 h-4" />
            </button>
          </div>
          <div className="flex justify-center gap-6 mt-4">
            <span className="text-[9px] text-zinc-600 uppercase font-black tracking-widest flex items-center gap-2">
              <Shield className="w-3 h-3" /> Secure Vault Active
            </span>
            <span className="text-[9px] text-zinc-600 uppercase font-black tracking-widest flex items-center gap-2">
              <UserCheck className="w-3 h-3" /> Digital Twin Verified
            </span>
          </div>
        </div>
      </div>
  ...
  const renderDrive = () => (
    <div className="p-8 space-y-6 overflow-y-auto h-full">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-black tracking-tight">Drive — Outputs</h2>
          <p className="text-xs text-zinc-500">{MOCK_DRIVE_FILES.length} files across missions</p>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input value={driveSearch} onChange={e => setDriveSearch(e.target.value)} placeholder="Search by filename or mission ID..." className="w-full bg-zinc-900 border border-zinc-800 rounded-xl pl-10 pr-4 py-3 text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:border-red-500" />
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-zinc-800">
              <th className="text-left text-[10px] text-zinc-500 uppercase tracking-widest font-bold px-5 py-3">File</th>
              <th className="text-left text-[10px] text-zinc-500 uppercase tracking-widest font-bold px-5 py-3">Mission</th>
              <th className="text-left text-[10px] text-zinc-500 uppercase tracking-widest font-bold px-5 py-3">Size</th>
              <th className="text-left text-[10px] text-zinc-500 uppercase tracking-widest font-bold px-5 py-3">Updated</th>
              <th className="text-right text-[10px] text-zinc-500 uppercase tracking-widest font-bold px-5 py-3">Action</th>
            </tr>
          </thead>
          <tbody>
            {filteredFiles.map(f => (
              <tr key={f.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30 transition-colors">
                ...
                <td className="px-5 py-3">
                  <span className="text-[10px] font-mono text-zinc-500">{f.missionId}</span>
                </td>
                <td className="px-5 py-3">
                  <span className="text-xs text-zinc-500">{f.size}</span>
                </td>
                <td className="px-5 py-3">
                  <span className="text-xs text-zinc-500">{f.updatedAt}</span>
                </td>
  ...
  const renderSecurity = () => (
    <div className="p-8 space-y-6 overflow-y-auto h-full">
      <div>
        <h2 className="text-lg font-black tracking-tight">Security Center</h2>
        <p className="text-xs text-zinc-500">Posture overview and audit trail</p>
      </div>

      {/* Posture Cards */}
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: 'Security Score', value: '96/100', icon: Shield, color: 'text-green-400' },
          { label: 'Active Threats', value: '0', icon: AlertTriangle, color: 'text-green-400' },
          { label: 'DLP Policies', value: '14', icon: Lock, color: 'text-blue-400' },
          { label: 'Last Audit', value: '2h ago', icon: Clock, color: 'text-zinc-400' },
        ].map(c => (
          <div key={c.label} className="bg-zinc-900/60 border border-zinc-800 rounded-2xl p-5">
            <c.icon className={`w-5 h-5 ${c.color} mb-3`} />
            <div className="text-xl font-black tracking-tight">{c.value}</div>
            <div className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold mt-1">{c.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-zinc-900/60 border border-zinc-800 rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-zinc-800 flex items-center justify-between">
          <h3 className="text-xs font-black uppercase tracking-widest text-zinc-400">Audit Log</h3>
          <RefreshCw className="w-3.5 h-3.5 text-zinc-600 cursor-pointer hover:text-zinc-400 transition-colors" />
        </div>
        <div className="divide-y divide-zinc-800/50">
          {MOCK_AUDIT_LOG.map(entry => (
            <div key={entry.id} className="px-5 py-3 flex items-center gap-4 hover:bg-zinc-800/20 transition-colors">
              {severityDot(entry.severity)}
              <span className="text-xs text-zinc-300 flex-grow">{entry.action}</span>
              <span className="text-[10px] text-zinc-600 font-mono shrink-0">{entry.timestamp}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-zinc-900/60 border border-zinc-800 rounded-2xl p-6">
        <h3 className="text-xs font-black uppercase tracking-widest text-zinc-400 mb-4">Credential Vault</h3>
        <div className="space-y-3">
          {[
            { name: 'Stripe API Key', rotated: '2h ago', status: 'fresh' },
            { name: 'Mailgun API Key', rotated: '6d ago', status: 'fresh' },
            { name: 'Twilio Auth Token', rotated: '14d ago', status: 'due' },
            { name: 'Meta Ads Token', rotated: '2d ago', status: 'fresh' },
            { name: 'Google Ads OAuth', rotated: '28d ago', status: 'overdue' },
          ].map(cred => (
            <div key={cred.name} className="flex items-center justify-between py-2">
              <div className="flex items-center gap-3">
                <Key className="w-4 h-4 text-zinc-500" />
                <span className="text-xs text-zinc-300">{cred.name}</span>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-[10px] text-zinc-600 font-mono">Rotated {cred.rotated}</span>
                <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${
                  cred.status === 'fresh' ? 'bg-green-600/20 text-green-400' :
                  cred.status === 'due' ? 'bg-amber-600/20 text-amber-400' :
  ...
  const renderKnowledge = () => (
    <div className="p-8 space-y-6 overflow-y-auto h-full">
      <div>
        <h2 className="text-lg font-black tracking-tight">Knowledge Browser</h2>
        <p className="text-xs text-zinc-500">Persistent Knowledge Base — {MOCK_KNOWLEDGE.length} entries</p>
      </div>
      ...
      <div className="flex gap-3">
        <div className="relative flex-grow">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-600" />
          <input value={kbSearch} onChange={e => setKbSearch(e.target.value)} placeholder="Search knowledge base..." className="w-full bg-zinc-900 border border-zinc-800 rounded-xl pl-10 pr-4 py-3 text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:border-red-500" />
        </div>
        <select value={kbTypeFilter} onChange={e => setKbTypeFilter(e.target.value)} className="bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-xs text-zinc-300 focus:outline-none focus:border-red-500">
          <option value="all">All Types</option>
          <option value="API_DOC">API Doc</option>
          <option value="DATA_SCHEMA">Data Schema</option>
      ...
      <div className="space-y-3">
        {filteredKB.map(entry => (
          <div key={entry.id} className="bg-zinc-900/60 border border-zinc-800 rounded-2xl p-5 hover:border-zinc-700 transition-colors">
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-3">
                <Database className="w-4 h-4 text-zinc-500" />
                <h4 className="text-sm font-bold text-white">{entry.title}</h4>
              </div>
              {knowledgeTypeBadge(entry.type)}
            </div>
            <p className="text-xs text-zinc-500 mb-3 pl-7">{entry.summary}</p>
            <div className="flex items-center gap-4 pl-7">
              <span className="text-[10px] text-zinc-600 font-mono">{entry.tokens.toLocaleString()} tokens</span>
              <span className="text-[10px] text-zinc-600">Updated {entry.updatedAt}</span>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-red-500/30 overflow-hidden flex flex-col">
      <GlassmorphismNav />

      <div className="flex-grow pt-24 pb-0 flex overflow-hidden">
        {/* Icon Sidebar */}
        <div className="w-16 shrink-0 bg-zinc-950 border-r border-zinc-800 flex flex-col items-center py-6 gap-1">
          <div className="w-9 h-9 bg-red-600 rounded-xl flex items-center justify-center font-black italic text-sm mb-6">
            E
          </div>
          {TABS.map(tab => {
            const Icon = tab.icon
            ...
            return (
              <button
                ...
                className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all relative group ${
                  isActive ? 'bg-zinc-800 text-white' : 'text-zinc-600 hover:text-zinc-300 hover:bg-zinc-800/50'
                }`}
              >
                <Icon className="w-[18px] h-[18px]" />
                {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-red-600 rounded-r" />}
                {/* Tooltip */}
                <div className="absolute left-full ml-2 px-2 py-1 bg-zinc-800 rounded text-[10px] font-bold text-zinc-300 whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50">
                  {tab.label}
                </div>
              </button>
            )
