...
const AnimatedRecruitingDemo = ({ isActive }: { isActive: boolean }) => {
  const [candidates, setCandidates] = useState([
    { name: "Alex Chen", role: "Senior Dev", score: 0, status: "screening" },
    { name: "Sarah Jones", role: "Product Mgr", score: 0, status: "screening" },
    { name: "Mike Ross", role: "Sales Lead", score: 0, status: "screening" },
  ])
    ...
    candidates.forEach((_, index) => {
      setTimeout(() => {
        setCandidates(prev => prev.map((c, i) =>
          i === index ? { ...c, status: "analyzing" } : c
        ))

        const targetScore = [98, 75, 88][index]
        let currentScore = 0
        const interval = setInterval(() => {
          currentScore += 5
          if (currentScore >= targetScore) {
            ...
            setCandidates(prev => prev.map((c, i) =>
              i === index ? { ...c, score: currentScore, status: "scored" } : c
            ))
          } else {
            setCandidates(prev => prev.map((c, i) =>
              i === index ? { ...c, score: currentScore } : c
            ))
          }
        }, 50)
  ...
  return (
    <div className="bg-slate-50 rounded-lg p-4 h-32 overflow-hidden flex flex-col gap-2">
      {candidates.map((c, i) => (
        <div key={i} className="flex items-center justify-between bg-white p-2 rounded border border-slate-100 shadow-sm transition-all duration-300">
          <div>
...
const AnimatedKnowledgeDemo = ({ isActive }: { isActive: boolean }) => {
  ...
  return (
    <div className="bg-slate-50 rounded-lg p-4 h-32 flex flex-col gap-3">
      <div className="bg-white p-2 rounded border border-slate-200 flex items-center gap-2 shadow-sm">
        ...
        <div className="text-xs text-slate-800">{query}<span className="animate-pulse">|</span></div>
      </div>
...
const AnimatedCommandDemo = ({ isActive }: { isActive: boolean }) => {
  const [steps, setSteps] = useState<string[]>([])

  useEffect(() => {
    if (!isActive) return

    const plan = ["Analyze competitors", "Draft Q3 content", "Schedule posts"]
...
const features = [
  ...
  {
    title: "AI Recruiter",
    description: "Automatically screen resumes, score candidates, and book interviews with top talent.",
    demo: AnimatedRecruitingDemo,
    size: "large",
  },
  {
    title: "Enterprise Knowledge Base",
    description: "Instantanswers for your team from your own documents and SOPs.",
    demo: AnimatedKnowledgeDemo,
    size: "medium",
  },
  {
    title: "Business Command Center",
    description: "Turn simple text commands into executed strategies and comprehensive plans.",
    demo: AnimatedCommandDemo,
    size: "medium",
  },
]
