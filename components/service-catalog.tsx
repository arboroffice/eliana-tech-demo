            ...
            <div className="max-w-7xl mx-auto relative z-10">
                <div className="text-center mb-16">
                    <h2 className="text-4xl md:text-5xl font-bold mb-6">
                        Complete Service Catalog for <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-red-400">Business Growth</span>
                    </h2>
                        ...
                        {phases.map((phase, index) => (
                            <div
                                ...
                                className={`rounded-2xl p-8 border transition-all duration-300 hover:scale-105 ${phase.highlight
                                    ? "bg-white/10 border-red-500/50 shadow-2xl shadow-red-500/20"
                                    : "bg-white/5 border-white/10 hover:border-white/20"
                                    }`}
                            >
                                <div className="mb-4">
                                    <span className="text-sm font-medium text-red-400">{phase.subtitle}</span>
                                ...
                                <ul className="space-y-3 mb-8">
                                    {phase.features.map((feature, i) => (
                                        <li key={i} className="flex items-start text-sm text-slate-300">
                                            <Check className="w-5 h-5 text-red-400 mr-2 flex-shrink-0" />
                                            {feature}
                                        </li>
                                    ))}
                                </ul>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {departments.map((dept, index) => (
                            <div key={index} className="bg-white/5 border border-white/10 rounded-xl p-6 hover:bg-white/10 transition-colors">
                                <h3 className="text-xl font-bold mb-4 text-red-300">{dept.name}</h3>
                ...
                <div className="mt-16 text-center">
                    ...
                    <div className="flex flex-wrap justify-center gap-4">
                        ...
                        <div className="bg-gradient-to-r from-red-900/40 to-red-900/40 px-6 py-4 rounded-lg border border-red-500/30">
                            <div className="text-sm text-red-300">Scale Partner</div>
                        </div>
                        <div className="bg-white/5 px-6 py-4 rounded-lg border border-white/10">
